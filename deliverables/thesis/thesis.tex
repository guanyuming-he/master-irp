...
\begin{enumerate}
	...
	\item Build and train a LLM. That is a lot of work I cannot afford to do.
		This project will use trained LLMs.
\end{enumerate}
...
\subsubsection{TODO: Other works focused on search engine}
TODO: TBD.
...
However, the commercial search engines suffer from a few serious problems.
\begin{enumerate}
	\item 
...
way. For each, a different system prompt will be used, carefully
...
\subsubsection{Results delivery} 
The results are sent to the users automatically via email. Because the code is
visible in the repository along with the login credentials, I registered a
burner Google account for email delivery. The configuration records a list of
destination email addresses, to each of which the final results of running the
pipeline will be sent from the burner account.

The protocol is SMTP + TLS to ensure that the email content is confidential.


\section{Results}
This section demonstrates what a user can achieve using my tool. Next, using
the same inputs, information retrieved by my tool and other mainstream LLMs \&
search engines are compared.


\section{Discussion}
This section discusses the results. I present the most significant challenges
that I encountered and how I addressed them; I discuss what methods achieved
desired results and what have not; I analyze 
Finally, I envisage how this tool can be further improved beyond this
three-month project.
